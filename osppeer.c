...
typedef struct task {
	...
	md5_state_t *md5_state;			// download MD5 state
...
taskbufresult_t read_to_taskbuf(int fd, task_t *t)
{
	...
	// append download data to MD5 hash
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	// obtain MD5 checksum from tracker
	osp2p_writef(tracker_task->peer_fd, "MD5SUM %s\n", t->filename);
	messagepos = read_tracker_response(tracker_task);
	if (tracker_task->buf[messagepos] != '2') {
		error("* Tracker error message while requesting MD5 checksum\n");
		goto exit;
	}
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	// Attack 1: attempt buffer overflow attack
	if (evil_mode == 1) {
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	if (t->total_written > 0) {
		...
		memset(md5, 0, MD5_TEXT_DIGEST_SIZE+1);
		if (md5_finish_text(t->md5_state, md5, 1) == MD5_TEXT_DIGEST_SIZE &&
			strncmp(t->checksum, md5, MD5_TEXT_DIGEST_SIZE) == 0)
			message("* File verification successful for '%s'\n", t->filename);
		else {
...
static void task_upload(task_t *t)
{
	...
	while (1) {
		...
		if (ret == TBUF_ERROR) {
			if (evil_mode == 3)
				message("* Attack successful. Peer write error.");
			else
		...
		if (ret == TBUF_ERROR) {
			...
			/* End of file */
			if (evil_mode == 3)
				// resets file pointer to beginning of file to keep writing
				lseek(t->disk_fd, 0, 0);
			else
				break;
		}
